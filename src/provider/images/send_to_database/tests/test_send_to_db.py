...
def test_get_df_valid_file(mocker, mock_db_session):
    ...
    data_sender = DataSender(docs_links_tables=[],
                             db_engine=mock_db_session, data_path=data_path)
...
def test_send_df_to_database_failure(mock_db_session):
    ...
    mock_db_session.commit = Mock(
        side_effect=SQLAlchemyError, spec=mock_db_session().commit)
...
def test_keep_bad_data_df(mocker, mock_db_session):
    ...
    data_sender = DataSender(docs_links_tables=[
    ], db_engine=mock_db_session, data_path=data_path, dir_bad_data=dir_bad_data)
    ...
    os.makedirs.assert_called_once_with(
        os.path.join(data_path, dir_bad_data), exist_ok=True)
    pd.DataFrame.to_csv.assert_called_once_with(os.path.join(
        data_path, dir_bad_data, "test_bad_data.csv"), index=False)
...
def test_call_process(mocker, mock_db_session):
    ...
    data_sender = DataSender(docs_links_tables=docs_links_tables,
                             db_engine=mock_db_session, data_path=data_path)
